# CSCI571_Web_Technologies
This repository showcases my course projects for CSCI571 Web Technologies @ USC. Due to the instructor's requirements, I am unable to publicly share the code for each project. Instead, this repository only includes sample videos of the projects. If you wish to access the source code, please contact me via email ([jiayiz99@usc.edu](mailto:jiayiz99@usc.edu), [zhangjiayi1512@outlook.com](mailto:zhangjiayi1512@outlook.com)).
## Assignment 1
Create a webpage according to a given screen recording using HTML and CSS. The project result is demonstrated in the video.

## Assignment 2
A simple weather query website built with the following technologies:
- Server-side
  - Python + Flask framework
  - Weather data: [Tomorrow.io](https://app.tomorrow.io/) RESTAPI
- Client-side
  - HTML, CSS and JavaScript
  - Geolocation: [ipinfo.io](https://ipinfo.io/) and [Google Maps Platform, Geocoding API](https://developers.google.com/maps/documentation/geocoding/start)
  - Weather data visualization: [highcharts](https://www.highcharts.com/)
The result is shown in the video, and the website was deployed on Google App Engine.

## Assignment 3
A responsive weather query website built with the following technologies:
- Server-side
  - Node.js + Express framework
  - Weather data: [Tomorrow.io](https://app.tomorrow.io/) RESTAPI
  - Saving favorite locations: [MongoDB](https://www.mongodb.com)
- Client-side
  - [Angular v19](https://angular.dev/)
  - [ng-Bootstrap](https://ng-bootstrap.github.io/#/home)
  - Geolocation: [ipinfo.io](https://ipinfo.io/)和[Google Maps Platform, Geocoding API](https://developers.google.com/maps/documentation/geocoding/start)
  - Autocomplete for location search: [Maps JavaScript API](https://developers.google.com/maps/documentation/javascript/place-autocomplete)
  - Weather data visualization: [Highcharts Angular]([https://www.highcharts.com/](https://www.npmjs.com/package/highcharts-angular))
The result is shown in the video, and the website was deployed on Google App Engine.

## Assignment 4
A weather query application for the Android platform based on Android API 35, featuring:
- Weather query on app launch for the current location
  - Weather data: [Tomorrow.io](https://app.tomorrow.io/) RESTAPI
  - Geolocation: [Location APIs](https://developer.android.com/develop/sensors-and-location/location) in Google Play services
- Autocomplete for location search: [Maps JavaScript API](https://developers.google.com/maps/documentation/javascript/place-autocomplete)
- Saving favorite locations: [MongoDB](https://www.mongodb.com)
- Weather data visualization: [Highcharts Android]([https://www.highcharts.com/](https://www.highcharts.com/blog/tutorials/highcharts-android-wrapper-tutorial/))
The result is shown in the video.

---
本仓库展示了我在CSCI571 Web Technologies @ USC的课程项目。由于授课教授的要求，我无法公开分享每个项目的代码，本仓库仅展示课程项目的样例视频。如果你希望获取源码，请通过邮件联系我（[jiayiz99@usc.edu](mailto:jiayiz99@usc.edu), [zhangjiayi1512@outlook.com](mailto:zhangjiayi1512@outlook.com)）。
## 作业一
根据给定的网页录屏通过HTML和CSS还原网页，作业结果如视频所示。

## 作业二
简单的天气查询网站，基于如下技术框架：
- 服务器端
  - Python + Flask框架
  - 天气数据获取：[Tomorrow.io](https://app.tomorrow.io/) RESTAPI
- 客户端
  - HTML，CSS和JavaScript
  - 地理位置信息获取：[ipinfo.io](https://ipinfo.io/)和[Google Maps Platform, Geocoding API](https://developers.google.com/maps/documentation/geocoding/start)
  - 天气数据可视化：[Highcharts](https://www.highcharts.com/)
实现结果如视频所示，网页被部署在Google App Engine上。

## 作业三
自适应的天气查询网站，基于如下技术框架：
- 服务器端
  - Node.js + Express框架
  - 天气数据获取：[Tomorrow.io](https://app.tomorrow.io/) RESTAPI
  - 收藏常用地理位置：[MongoDB](https://www.mongodb.com)
- 客户端
  - [Angular v19](https://angular.dev/)
  - [ng-Bootstrap](https://ng-bootstrap.github.io/#/home)
  - 地理位置信息获取：[ipinfo.io](https://ipinfo.io/)和[Google Maps Platform, Geocoding API](https://developers.google.com/maps/documentation/geocoding/start)
  - 地理位置查询自动补全：[Maps JavaScript API](https://developers.google.com/maps/documentation/javascript/place-autocomplete)
  - 天气数据可视化：[Highcharts Angular](https://www.npmjs.com/package/highcharts-angular)
实现结果如视频所示，网页被部署在Google App Engine上。

## 作业四
安卓平台的天气查询应用，基于Android API 35，实现了如下功能：
- 开屏查询当前位置天气
  - 天气数据获取：[Tomorrow.io](https://app.tomorrow.io/) RESTAPI
  - 地理位置信息获取：[Location APIs](https://developer.android.com/develop/sensors-and-location/location) in Google Play services
- 地理位置查询自动补全：[Maps JavaScript API](https://developers.google.com/maps/documentation/javascript/place-autocomplete)
- 收藏常用地理位置：[MongoDB](https://www.mongodb.com)
- 天气数据可视化：[Highcharts Android](https://www.highcharts.com/blog/tutorials/highcharts-android-wrapper-tutorial/)
实现结果如视频所示。

https://github.com/user-attachments/assets/420d72a2-b921-4f4a-a6d8-65b9f5e2a71c


https://github.com/user-attachments/assets/4b290a23-e836-4424-8394-afc153a71df7


https://github.com/user-attachments/assets/d2fdeaab-026c-4545-824c-3095fa5fa038


https://github.com/user-attachments/assets/c3a0b653-1b6c-4f93-9910-74a04efcaf52


https://github.com/user-attachments/assets/efc5fc15-46ae-4840-907c-5bd1c87d97d7
